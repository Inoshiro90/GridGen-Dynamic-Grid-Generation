...
function determineSide(point, dimension) {
    if (point === dimension.C1 || point === dimension.C2 || point === dimension.C3 || point === dimension.C4) return 'corner';
...
function createSingleTable(dimension, tableData, container, tableNumber, xMax, yMax, corners) {
    ...
    for (let i = 1; i <= dimension.pointTotal; i++) {
        ...
        const cells = [
            i, startSide, startCoords.x, startCoords.y,
            endPoint, endSide, endCoords.x, endCoords.y
        ];
